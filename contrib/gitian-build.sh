...
VERSION=
commit=false
url=https://github.com/betfint/betfint
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/betfint/betfint
...
then
	# Make output folder
	mkdir -p ./betfint-binaries/${VERSION}
	...
	then
	    ...
	    mv build/out/betfint-*.tar.gz build/out/src/betfint-*.tar.gz ../betfint-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    mv build/out/betfint-*.zip build/out/betfint-*.exe ../betfint-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    mv build/out/betfint-*.tar.gz build/out/betfint-*.dmg ../betfint-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit betfint=${COMMIT} --url betfint=${url} ../betfint/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../betfint/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/betfint-*.tar.gz build/out/src/betfint-*.tar.gz ../betfint-binaries/${VERSION}
	popd
...
then
	...
	echo ""
	./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-aarch64 ../betfint/contrib/gitian-descriptors/gitian-aarch64.yml
	# Signed Windows
	echo ""
...
then
	...
	then
	    ...
	    mv build/out/betfint-*win64-setup.exe ../betfint-binaries/${VERSION}
	    mv build/out/betfint-*win32-setup.exe ../betfint-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    mv build/out/betfint-osx-signed.dmg ../betfint-binaries/${VERSION}/betfint-${VERSION}-osx.dmg
	fi
	popd
