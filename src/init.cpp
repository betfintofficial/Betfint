...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "betfint.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "betfintd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BETF and zBETF money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BETF/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BETF/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, betfint, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BETF/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Betfint specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-betfstake=<n>", strprintf(_("Enable or disable staking functionality for BETF inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbetfstake=<n>", strprintf(_("Enable or disable staking functionality for zBETF inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbetf=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBETF minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbetfbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBETF backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebetfintamount=<n>     " + strprintf(_("Keep N BETF anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Betfint Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Betfint is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Betfint Core is shutting down."));
...
#endif
    // Make sure only a single Betfint process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Betfint Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BETFINT: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BETFINT specific: zerocoin and spork DB's
                ...
                // BETFINT: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBETFMinted();
                        RecalculateZBETFSpent();
                    }
                    RecalculateBETFSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // BETFINT: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Betfint Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Betfint Core to complete") << "\n";
        ...
        zwalletMain = new CzBETFWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBETFWallet
        uiInterface.InitMessage(_("Syncing zBETF wallet..."));

        bool fEnableZBetfBackups = GetBoolArg("-backupzbetf", true);
        pwalletMain->setZBetfAutoBackups(fEnableZBetfBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbetfTracker->Init();
...
//    }
//
//    nAnonymizeBetfintAmount = GetArg("-anonymizebetfintamount", 0);
//    if (nAnonymizeBetfintAmount > 999999) nAnonymizeBetfintAmount = 999999;
//    if (nAnonymizeBetfintAmount < 2) nAnonymizeBetfintAmount = 2;
    ...
    LogPrintf("Anonymize Betfint Amount %d\n", nAnonymizeBetfintAmount);
    ...
    /* Denominations
       ...
       For example:
       1BETF+1000 == (.1BETF+100)*10
       10BETF+10000 == (1BETF+1000)*10
    */
