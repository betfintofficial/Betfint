...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zBETF spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zBETF spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zBETF serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        uint256 nCheckpoint;
        int x = nHeight - (nHeight % 10) - 10; //XXX
                int y = chainActive.Height(); //XXX
                if ( (x>=0 ) && (x<=y) ) {
                	 uint256 hashBlockLastAccumulated = chainActive[x]->GetBlockHash(); //XXX
        ...
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zBETF will be disabled and previous block's checkpoint is all that will be needed
        ...
        CAmount nExpectedMint = GetBlockValue(nHeight-1);
        CAmount nMNBetReward = 0;
        vExpectedPayouts = GetBetPayouts(nHeight- 1);
        nExpectedMint += GetBlockPayouts(vExpectedPayouts, nMNBetReward);
        nExpectedMint += nMNBetReward;


        pblock->vtx[0].vout[0].nValue = nExpectedMint; // XXX
        pblock->vtx[0].vin[0].scriptSig = scriptPubKeyIn; //XXX
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BetfintMiner : generated block is stale");
    }
...
{
    LogPrintf("BetfintMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("betfint-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zBETF with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zBETF key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zBETF key failed \n");
                    continue;
                }
        ...
        LogPrintf("Running BetfintMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
