...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBetfintAmount"))
        settings.setValue("nAnonymizeBetfintAmount", 1000);

    nAnonymizeBetfintAmount = settings.value("nAnonymizeBetfintAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBetfintAmount"))
        SoftSetArg("-anonymizebetfintamount", settings.value("nAnonymizeBetfintAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBetfintAmount:
            return QVariant(nAnonymizeBetfintAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBetfintAmount:
            nAnonymizeBetfintAmount = value.toInt();
            settings.setValue("nAnonymizeBetfintAmount", nAnonymizeBetfintAmount);
            emit anonymizeBetfintAmountChanged(nAnonymizeBetfintAmount);
            break;
        case CoinControlFeatures:
