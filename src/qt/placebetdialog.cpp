...
void PlaceBetDialog::on_placeBetButton_clicked()
{
    ...
    if (!ui->payAmount->validate() || ui->payAmount->value(0) < (Params().MinBetPayoutRange() * COIN) || ui->payAmount->value(0) > ( Params().MaxBetPayoutRange() * COIN ) ){
        ...
        QString questionString = tr("Bet amount must be between 50 - 10,000 BETF inclusive!");

        QPair<QString, CClientUIInterface::MessageBoxFlags> msgParams;
        // Default to a warning message, override if error message is needed
        msgParams.second = CClientUIInterface::MSG_WARNING;

        msgParams.first = tr("Bet amount must be between 50 - 10,000 BETF inclusive!");
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_BETF, true));
        if (!ctx.isValid()) {
...
void PlaceBetDialog::send(CAmount amount, const std::string& eventId, const std::string& teamToWin, const std::string& eventTime)
{
    ...
    //     printf("I accept your betfint good sir!! \n");
    //     accept();
    // }else{
    //     printf("I DO NOT ACCEPT your betfint good sir!! \n");
    // }
}
