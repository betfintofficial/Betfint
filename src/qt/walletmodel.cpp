...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered betfint address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BETF.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzBETFReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBETFReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBETFReset.connect(boost::bind(NotifyzBETFReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBETFReset.disconnect(boost::bind(NotifyzBETFReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbetfTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
