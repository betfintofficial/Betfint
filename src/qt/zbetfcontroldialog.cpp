...
#include "ui_zbetfcontroldialog.h"
...
std::set<std::string> ZBetfControlDialog::setSelectedMints;
std::set<CMintMeta> ZBetfControlDialog::setMints;

ZBetfControlDialog::ZBetfControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBetfControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBetfControlDialog::~ZBetfControlDialog()
{
    delete ui;
}

void ZBetfControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBetfControlDialog::updateList()
{
...
void ZBetfControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBetfControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBetf_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBetfControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBetfControlDialog::GetSelectedMints()
{
...
void ZBetfControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
