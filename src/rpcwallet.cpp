...
// TODO The Betfint functions in this file are being placed here for speed of
...
UniValue listbets(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"team-to-win\":\"betfintaddress\",  (string) The team to win.\n"
            "    \"amount\": x.xxx,                  (numeric) The amount bet in BETF.\n"
            "  }\n"
            "]\n"
...
void EnsureEnoughBetfint(CAmount total)
{
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"betfintaddress\"    (string) The new betfint address\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current Betfint address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"betfintaddress\"   (string) The account betfint address\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"betfintaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"betfintaddress\"  (string, required) The betfint address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"betfintaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"betfintaddress\"  (string, required) The betfint address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"betfintaddress\"  (string) a betfint address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue placebet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "3. \"amount\"      (numeric, required) The amount in betf to send. eg 0.1\n"
    ...
    // Validate bet amount so its between 1 - 10000 BETF inclusive.
    if( nAmount < (50 * COIN ) || nAmount > (10000 * COIN)){
        throw JSONRPCError(RPC_BET_DETAILS_ERROR, "Error: Incorrect bet amount. Please ensure your bet is beteen 1 - 10000 BETF inclusive.");
    }
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"betfintaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"betfintaddress\"  (string, required) The betfint address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in BETF to send. eg 0.1\n"
            "3. \"comment\"     (string, optional) A comment used to store what the transaction is for. \n"
            "                             This is not part of the transaction, just kept in your wallet.\n"
            "4. \"comment-to\"  (string, optional) A comment to store the name of the person or organization \n"
            "                             to which you're sending the transaction. This is not part of the \n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddressix \"betfintaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"betfintaddress\",     (string) The betfint address\n"
            "      amount,                 (numeric) The amount in BETF\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"betfintaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"betfintaddress\"  (string, required) The betfint address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"betfintaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given betfintaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"betfintaddress\"  (string, required) The betfint address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in BETF received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BETF received for this account.\n"
...
UniValue getextendedbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() > 0))
        throw runtime_error(
            ...
            "  \"balance\": \"xxx\", (string) The total BETF balance\n"
            "  \"balance_locked\": \"xxx\", (string) The locked BETF balance\n"
            "  \"balance_unlocked\": \"xxx\", (string) The unlocked BETF balance\n"
            "  \"balance_unconfirmed\": \"xxx\", (string) The unconfirmed BETF balance\n"
            "  \"balance_immature\": \"xxx\", (string) The immature BETF balance\n"
            "  \"zerocoin_balance\": \"xxx\", (string) The total zBETF balance\n"
            "  \"zerocoin_balance_mature\": \"xxx\", (string) The mature zBETF balance\n"
            "  \"zerocoin_balance_immature\": \"xxx\", (string) The immature zBETF balance\n"
            "  \"watchonly_balance\": \"xxx\", (string) The total watch-only BETF balance\n"
            "  \"watchonly_balance_unconfirmed\": \"xxx\", (string) The unconfirmed watch-only BETF balance\n"
            "  \"watchonly_balance_immature\": \"xxx\", (string) The immature watch-only BETF balance\n"
            "  \"watchonly_balance_locked\": \"xxx\", (string) The locked watch-only BETF balance\n"
            "}\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 BETF from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 BETF from timotei to akiko with a comment and funds have 6 confirmations\n" +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tobetfintaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a betfint address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tobetfintaddress\"  (string, required) The betfint address to send funds to.\n"
            "3. amount                (numeric, required) The amount in BETF. (transaction fee is added on top).\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 BETF from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The betfint address is the key, the numeric amount in BETF is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    BOOST_FOREACH(const string& name_, keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Betfint address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "2. \"keysobject\"   (string, required) A json array of betfint addresses or hex-encoded public keys\n"
            "     [\n"
            ...
            "\nResult:\n"
            "\"betfintaddress\"  (string) A betfint address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in BETF received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"betfintaddress\",    (string) The betfint address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BETF. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BETF. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"betfintaddress\",    (string) The betfint address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BETF. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BETF. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in BETF\n"
            ...
            "      \"address\" : \"betfintaddress\",   (string) The betfint address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in BETF\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BETFs\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending BETFs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"betfintaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; betfint server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending BETFs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in BETF/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total BETF balance of the wallet\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same Betfint address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <BETFINT Address> <percent>\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BETF address");
...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zBETF balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zBETF balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "listmintedzerocoins\n"
            "\nList all zBETF mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zbetfTracker->ListMints(true, false, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zbetfTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zBETF mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nMint the specified zBETF amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Betf to convert to zBETF\n"
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zBETF is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in BETF.\n"
            "      \"address\": \"xxx\"         (string) BETF address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zBETF
...
UniValue spendzerocoinfrom(const UniValue& params, bool fHelp)
{
    ...
    for (unsigned int inx = 0; inx < inputsPubcoinHashes.size(); inx++){
        ...
        CMintMeta mintMeta = pwalletMain->zbetfTracker->GetMetaFromPubcoin(pubcoinHash);
        ...
        if (mintMeta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            ...
            if (nSecurityLevel < 100)
                throw JSONRPCError(RPC_WALLET_ERROR, "Version 1 zBETF require a security level of 100 to successfully spend.");
        }
        CZerocoinMint mint;
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBETFTracker* zbetfTracker = pwalletMain->zbetfTracker.get();
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBETFTracker* zbetfTracker = pwalletMain->zbetfTracker.get();
    set<CMintMeta> setMints = zbetfTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zBETF\n"
            ...
            "    \"v\": version,   (numeric) The version of the zBETF\n"
            "    \"k\": \"privkey\"  (string) The zBETF private key (V2+ zBETF only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzBETFTracker* zbetfTracker = pwalletMain->zbetfTracker.get();
    set<CMintMeta> setMints = zbetfTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zBETF value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zbetfTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zBETF list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzbetfseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
            "setzbetfseed \"seed\"\n"
            "\nSet the wallet's deterministic zbetf seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zbetf seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzbetfseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzbetfseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzBETFWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzbetfseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzbetfseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zBETF seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzbetfseed", "") + HelpExampleRpc("getzbetfseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zBETF seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zBETF to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zBETF to generate.\n"
    ...
    CzBETFWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzbetfstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzbetfstate\n"
                        "\nThe current state of the mintpool of the deterministic zBETF wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzbetf_count", nCount));
...
void static SearchThread(CzBETFWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZBETF(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzbetf(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzbetf\n"
            "\nMake an extended search for deterministically generated zBETF that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zBETF to start with.\n"
            "2. \"range\"       (numeric) How many zBETF to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzbetf", "1, 100, 2") + HelpExampleRpc("searchdzbetf", "1, 100, 2"));
    ...
    boost::thread_group* dzbetfThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzbetfThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzbetfThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zbetfTracker->GetSerialHashes());
