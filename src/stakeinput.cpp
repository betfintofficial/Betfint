...
bool CZWgrStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (!pwallet->CreateZBETFOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBETF output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBETF", __func__);

    for (unsigned int i = 0; i < 1; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBETFOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBETF output", __func__);
