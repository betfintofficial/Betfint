...
int nAnonymizeBetfintAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            if (ptrCategory->count(string("betfint"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BETFINT
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BETFINT
// Mac: ~/Library/Application Support/BETFINT
// Unix: ~/.betfint
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BETFINT";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "betfint.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty betfint.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "betfintd.pid"));
