...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 BETF at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBETF send
enum ZerocoinSpendStatus {
    ZBETF_SPEND_OKAY = 0,                            // No error
    ZBETF_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBETF_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBETF_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBETF_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBETF_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBETF_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBETF_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBETF_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBETF_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBETF_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBETF_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBETF_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBETF_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBETF_SPENT_USED_ZBETF = 14,                      // Coin has already been spend
    ZBETF_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBETF_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    */
    ...
    /** zBETF reset */
    boost::signals2::signal<void()> NotifyzBETFReset;
