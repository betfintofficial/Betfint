...
public:
    ...
    bool WriteZBETFSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBETFSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBETFSeed_deprecated(uint256& seed);
    bool EraseZBETFSeed();
    bool EraseZBETFSeed_deprecated();

    bool WriteZBETFCount(const uint32_t& nCount);
    bool ReadZBETFCount(uint32_t& nCount);
