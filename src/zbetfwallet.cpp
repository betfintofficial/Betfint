...
CzBETFWallet::CzBETFWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbetf seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBETFSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBETFSeed_deprecated()) {
                    LogPrintf("%s: Updated zBETF seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbetf seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbetf wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBETFWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBETFCount(nCountLastUsed);
    else if (!walletdb.ReadZBETFCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBETFWallet::Lock()
{
    seedMaster = 0;
}

void CzBETFWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBETFWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBETF(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBETFWallet::LoadMintPoolFromDB()
{
...
void CzBETFWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBETFWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBETFWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbetfTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBETFWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBETF(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbetfTracker which also adds to database
    pwalletMain->zbetfTracker->Add(dMint, true);
...
void CzBETFWallet::SeedToZBETF(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBETFWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBETFWallet::GenerateDeterministicZBETF(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBETFWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBETFWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
