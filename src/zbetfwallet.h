...
#ifndef BETFINT_ZBETFWALLET_H
#define BETFINT_ZBETFWALLET_H
...
public:
    CzBETFWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBETF(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBETF(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BETFINT_ZBETFWALLET_H
