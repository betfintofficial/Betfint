...
CzBETFTracker::CzBETFTracker(std::string strWalletFile)
{
...
CzBETFTracker::~CzBETFTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBETFTracker::Init()
{
...
bool CzBETFTracker::Archive(CMintMeta& meta)
{
...
bool CzBETFTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBETFTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBETFTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBETFTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBETFTracker::GetSerialHashes()
{
...
CAmount CzBETFTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBETFTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBETFTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBETFTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBETFTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBETFTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBETFTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBETFTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBETFTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBETFTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBETFTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBETFTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBETFTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBETFTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBETFTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBETFTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBETFTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbetf from DB\n", __func__, listDeterministicDB.size());
    }
